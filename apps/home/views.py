...
def home(request):
    if request.method == 'GET':
        ...
        item_list_doing = ItemList.objects.filter(due_date=atual_date).filter(status='DOING').order_by('-due_date')
        item_list_done = ItemList.objects.filter(type='A').filter(status='DONE').order_by('-due_date')
...
def doing(request):
    ...
    if request.method == 'GET':
        if request.user.is_authenticated:
            ...
            return render(request, 'pages/doing.html', context={'item_list_doing': item_list_doing})
...
def done(request):
    if request.method == 'GET':
        if request.user.is_authenticated:
            person = Person.objects.get(user=request.user)
            item_list_done = person.item_list.filter(status='DONE').order_by('-due_date')
            return render(request, 'pages/done.html', context={'item_list_done':item_list_done})
